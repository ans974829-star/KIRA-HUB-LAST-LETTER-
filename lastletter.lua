-- 🔥 KIRA HUB [Last Letter] v3.2 — ABSOLUTE FINAL EDITION 🔥
-- Made by KIRA
...
KeyGui.Parent = CoreGui or gethui()
...
KeyNote.Text = "🔑 Enter Key to Continue"
...
DiscordButton.Text = "💬 COPY DISCORD INVITE (Get Key)"
...
YouTubeButton.Text = "📺 COPY YOUTUBE LINK (Subscribe!)"
...
DiscordButton.MouseButton1Click:Connect(function()
    ...
    StarterGui:SetCore("SendNotification", {Title = "📋 Copied!", Text = "Discord invite copied!", Duration = 5})
    KeyStatus.Text = "✅ Discord link copied!"
...
YouTubeButton.MouseButton1Click:Connect(function()
    ...
    StarterGui:SetCore("SendNotification", {Title = "📋 Copied!", Text = "YouTube link copied!", Duration = 5})
    KeyStatus.Text = "✅ YouTube link copied!"
    KeyStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
end)
...
SubmitButton.MouseButton1Click:Connect(function()
    ...
    if key == VALID_KEY then
        ...
        KeyStatus.Text = "❌ Invalid Key! Join Discord for key."
        KeyStatus.TextColor3 = Color3.fromRGB(255, 100, 100)
        TweenService:Create(KeyInputStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(255, 50, 50)}):Play()
...
local TYPING_SPEEDS = {
    {name = "🐌 SNAIL", delay = 0.25, randomMin = 20, randomMax = 35, isHuman = false},
    {name = "👤 HUMAN", delay = 0.10, randomMin = 8, randomMax = 15, isHuman = false},
    {name = "⚡ MASTER", delay = 0.05, randomMin = 3, randomMax = 8, isHuman = false},
    {name = "💻 SUPER", delay = 0.02, randomMin = 1, randomMax = 3, isHuman = false},
    {name = "🧠 REAL", delay = 0.12, randomMin = 10, randomMax = 20, isHuman = true}, -- NEW!
}
...
local CloseButton = CreateCtrlBtn("✕", UDim2.new(1, -40, 0, 17), Color3.fromRGB(200, 50, 50))
local MinimizeButton = CreateCtrlBtn("—", UDim2.new(1, -75, 0, 17), Color3.fromRGB(70, 70, 70))
...
local AutoTypeToggle = CreateToggle("AutoType", "⚡ AUTO-TYPE", UDim2.new(0, 0, 0, 2), false, 0.25)
local AutoSuggestToggle = CreateToggle("AutoSuggest", "💡 SUGGEST", UDim2.new(0.25, 0, 0, 2), true, 0.25)
local MeaningfulToggle = CreateToggle("Meaningful", "✓ REAL", UDim2.new(0.50, 0, 0, 2), true, 0.25)
local HardWordsToggle = CreateToggle("HardWords", "💀 HARD", UDim2.new(0.75, 0, 0, 2), false, 0.25)

-- ═══════════════════════════════════════════════════════════════
-- ⚡ TYPING SPEED (WITH REAL HUMAN!)
-- ═══════════════════════════════════════════════════════════════
...
SpeedTitle.Text = "⌨️ TYPING SPEED (🧠 REAL = Human Mistakes!)"
...
DelayTitle.Text = "⏱️ THINKING DELAY (Wait before typing)"
...
RangeTitle.Text = "📏 AUTO-TYPE RANGE"
...
LettersSubtext.Text = "⏳ Waiting for letters..."
...
SuggestionsTitle.Text = "📝 AUTO-SUGGESTIONS"
...
CreditsFrame.Size = UDim2.new(1, 0, 0, 22)
...
CreditsLabel.Text = "Made by KIRA 🔥 | YT: @kira_0001 | DC: discord.gg/ZH48pK5wen"
...
local function Notify(title, msg, dur)
    pcall(function()
        StarterGui:SetCore("SendNotification", {Title = title or "🔥 KIRA", Text = msg, Duration = dur or 3})
    end)
end
...
AutoTypeToggle.MouseButton1Click:Connect(function()
    ...
    Notify("⚡ Auto-Type", AutoTypeEnabled and "ENABLED" or "DISABLED", 2)
end)
...
AutoSuggestToggle.MouseButton1Click:Connect(function()
    ...
    Notify("💡 Auto-Suggest", AutoSuggestEnabled and "ENABLED" or "DISABLED", 2)
end)
...
MeaningfulToggle.MouseButton1Click:Connect(function()
    ...
    Notify("✓ Real Words", MeaningfulOnlyMode and "ONLY REAL" or "ALL", 2)
end)
...
HardWordsToggle.MouseButton1Click:Connect(function()
    ...
    Notify("💀 Hard Words", HardWordsMode and "X,Z,Q,J PRIORITY" or "NORMAL", 2)
end)
...
for i, btn in ipairs(SpeedButtons) do
    btn.MouseButton1Click:Connect(function()
        ...
        if speed.isHuman then
            Notify("🧠 REAL HUMAN", "Makes typos & corrects them!", 3)
        else
            Notify("⌨️ Speed", speed.name, 2)
        end
    end)
end
...
for i, btn in ipairs(DelayButtons) do
    btn.MouseButton1Click:Connect(function()
        ...
        UpdateDelayButtons()
        Notify("⏱️ Thinking Delay", THINKING_DELAYS[i].name .. " before typing", 2)
    end)
end
...
for i, btn in ipairs(RangeButtons) do
    btn.MouseButton1Click:Connect(function()
        ...
        UpdateRangeButtons()
        Notify("📏 Range", WordRangeOptions[i].name, 2)
    end)
end
...
-- ═══════════════════════════════════════════════════════════════
-- 🧠 REAL HUMAN TYPING (Makes mistakes & corrects!)
-- ═══════════════════════════════════════════════════════════════
...
local function AutoTypeWord(word, input)
    ...
    task.spawn(function()
        ...
        if thinkDelay > 0 then
            LettersSubtext.Text = "🤔 Thinking... (" .. thinkDelay .. "s)"
            task.wait(thinkDelay)
        end
        
        LettersSubtext.Text = "⌨️ Typing..."
        ...
        -- Small pause before enter (human hesitation)
        if speed.isHuman then
        ...
        LettersSubtext.Text = "✅ Typed: " .. word:upper()
    end)
end
...
function UpdateSuggestions(input)
    ...
    LettersSubtext.Text = "🔍 Found suggestions below!"
...
local function LoadDictionary()
    ...
    Notify("✅ Loaded", #verifiedWords .. " words!", 3)
end
        ...
        if letters then
            ...
            if AutoTypeEnabled then
                ...
                if not best then
                    ...
                    if best then Notify("⚠️ Fallback", best:upper(), 2) end
                end

                if best then
                    local speed = TYPING_SPEEDS[SelectedSpeedIndex]
                    local prefix = isHard and "💀 " or (speed.isHuman and "🧠 " or "⚡ ")
                    Notify(prefix .. best:upper(), #best .. " chars", 2)
                    AutoTypeWord(best, letters:lower())
                else
                    Notify("❌ No Word", "Not found!", 2)
                end
            end
        end
    end
...
task.wait(0.8)
Notify("🔥 KIRA [Last Letter] v3.2", "FINAL EDITION Loaded!", 5)

task.wait(2)
Notify("Made by KIRA 🔥", "YT: @kira_0001 | DC: discord.gg/ZH48pK5wen", 8)

print("═══════════════════════════════════════════════════")
print("🔥 KIRA HUB [Last Letter] v3.2 — ABSOLUTE FINAL")
print("═══════════════════════════════════════════════════")
print("🧠 NEW: REAL HUMAN mode (makes typos & corrects!)")
print("⏱️ NEW: Thinking Delay (0s, 0.5s, 1s, 2s, 5s)")
print("═══════════════════════════════════════════════════")
print("📺 YouTube: @kira_0001")
print("💬 Discord: discord.gg/ZH48pK5wen")
print("═══════════════════════════════════════════════════")
